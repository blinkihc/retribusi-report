  ...
  return (
    <PullToRefresh onRefresh={handleRefresh}>
        ...
        {showFilters && (
          <div className="mt-6 rounded-lg border-2 border-black bg-slate-50 p-6 shadow-hard-sm animate-in slide-in-from-top-2 duration-200">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {/* OPD Filter */}
              <div>
                <label htmlFor="opd-filter" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">
                  OPD
                </label>
                <select
                  id="opd-filter"
                ...
                >
                  ...
                  {/* TODO: Map OPD options */}
                </select>
              </div>

              {/* Jenis Retribusi Filter */}
              <div>
                <label htmlFor="jenis-filter" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">
                  Jenis Retribusi
                </label>
                <select
                  id="jenis-filter"
                ...
                >
                  ...
                  {/* TODO: Map Jenis Retribusi options */}
                </select>
              </div>

              {/* Start Date */}
              <div>
                <label htmlFor="start-date" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">
                  Dari Tanggal
                </label>
                <input
                  type="date"
              ...
              {/* End Date */}
              <div>
                <label htmlFor="end-date" className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">
                  Sampai Tanggal
                </label>
                <input
                  type="date"
      ...
      <div className="px-4 lg:px-8 w-full max-w-full">
        <div
          className="rounded-lg border-2 border-black bg-white shadow-hard-lg overflow-hidden"
        >
          {isLoading ? (
            <div className="flex flex-col items-center justify-center p-24 bg-slate-50">
              <div className="h-12 w-12 animate-spin rounded-full border-4 border-solid border-primary-600 border-t-transparent mb-4"></div>
              <p className="text-lg font-bold text-slate-900 uppercase tracking-wide">Memuat Data...</p>
            </div>
          ) : (
              ...
              <div className="lg:hidden divide-y-2 divide-slate-200">
                {data?.data.length === 0 ? (
                  ...
                  data?.data.map((laporan) => (
                    <div key={laporan.id} className="p-4 bg-white hover:bg-yellow-50 transition-colors group">
                      {/* Card Header */}
                      ...
                      <div className="space-y-2 mb-4">
                        <div>
                          <div className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">OPD</div>
                          <div className="text-sm font-medium text-slate-900">{laporan.opdNama}</div>
                        </div>
                        <div>
                          <div className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">Jenis Retribusi</div>
                          <div className="text-sm font-medium text-slate-900">{laporan.jenisRetribusiNama}</div>
                        </div>
                        <div className="flex items-center justify-between pt-2 border-t-2 border-dashed border-slate-200 mt-2">
                          <div>
                            <div className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">Tanggal Setor</div>
                            <div className="text-sm font-medium text-slate-900">{formatDate(laporan.tanggalSetor)}</div>
                          </div>
                          <div className="text-right">
                            <div className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">Nominal</div>
                            <div className="text-lg font-extrabold text-slate-900">{formatCurrency(Number(laporan.nominal))}</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t-2 border-slate-100">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            type="button"
                            onClick={() => handleDownloadPDF(laporan.id, laporan.nomorLaporan)}
              ...
              <div className="hidden lg:block overflow-x-auto">
                <table className="w-full border-collapse text-left" style={{ minWidth: '1200px' }}>
                  <thead className="bg-slate-900 text-white">
                    <tr>
                      <th
                        className="px-6 py-4 text-xs font-bold uppercase tracking-wider border-r border-slate-700 cursor-pointer hover:bg-slate-800 transition-colors"
                      ...
                      <th
                        className="px-6 py-4 text-xs font-bold uppercase tracking-wider border-r border-slate-700 cursor-pointer hover:bg-slate-800 transition-colors text-right"
                      ...
                      <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider border-r border-slate-700 text-center">
                        Bukti
                      </th>
                      <th
                        className="px-6 py-4 text-xs font-bold uppercase tracking-wider border-r border-slate-700 cursor-pointer hover:bg-slate-800 transition-colors text-center"
              ...
              {data && data.pagination.totalPages > 1 && (
                <div className="border-t-2 border-black bg-white px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="text-sm font-medium text-slate-600">
                    Menampilkan <span className="font-bold text-black">{(data.pagination.page - 1) * data.pagination.limit + 1}</span> - <span className="font-bold text-black">{Math.min(data.pagination.page * data.pagination.limit, data.pagination.total)}</span> dari <span className="font-bold text-black">{data.pagination.total}</span> data
                  </div>

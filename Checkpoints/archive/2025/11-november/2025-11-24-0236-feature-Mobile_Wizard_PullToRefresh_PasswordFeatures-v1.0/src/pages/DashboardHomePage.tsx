...
import { ArrowRight, BarChart3, Calendar, DollarSign, FileText, PieChart as PieChartIcon, TrendingDown, TrendingUp } from 'lucide-react'
...
const GrowthIndicator = ({ value }: { value: number }) => {
  ...
  if (value === 0) return <span className="text-slate-400 font-bold text-[10px] uppercase tracking-wide">Tidak ada perubahan</span>

  return (
    <div className={`flex items-center text-[10px] font-bold uppercase tracking-wide ${isPositive ? 'text-green-600' : 'text-red-600'}`}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 border-b-2 border-dashed border-slate-300 pb-6">
        <div>
          ...
          <p className="text-slate-600 font-medium">
            Overview kinerja penerimaan retribusi daerah
          </p>
        </div>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                ...
                <Legend 
                  ...
                  height={36} 
                  formatter={(value) => <span className="text-xs font-bold uppercase text-slate-600">{value}</span>}
                />
              </PieChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="p-6 flex-1 min-h-[400px]">
            <ResponsiveContainer width="100%" height={350}>
              ...
              >
                ...
                />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="total" fill="#2563eb" stroke="#000000" strokeWidth={2} radius={[4, 4, 0, 0]} barSize={40}>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs font-bold text-slate-500 mt-1 uppercase tracking-wider">
                        {report.jenisRetribusi?.nama} • <span className="text-slate-400">{report.opd?.nama}</span>
                      </div>
                    </div>
                  </div>

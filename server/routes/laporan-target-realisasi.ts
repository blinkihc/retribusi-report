...
async function getRealisasiBulanan(tahun: number, jenisIds: number[]) {
  ...
  return db
    ...
    .groupBy(
      laporanRetribusi.jenisRetribusiId,
...
laporanTargetRealisasiRouter.get('/rekap', async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      tahun,
      bulanSekarang,
      namaBulan: BULAN_NAMES[bulanSekarang],
    })
  } catch (error) {
    next(error)
  }
    ...
    if (tabel === 'matrix') {
      ...
      data = targets.map((t) => {
        ...
        return {
          ...
          realisasiBulanIni,
          realisasiTotal,
          persentase: target > 0 ? Math.round((realisasiTotal / target) * 10000) / 100 : 0,
          sisaTarget: target - realisasiTotal,
        }
      })
    }

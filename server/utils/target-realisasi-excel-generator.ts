...
const IDR = (n: number) =>
    new Intl.NumberFormat('id-ID', { style: 'decimal', minimumFractionDigits: 0 }).format(Math.round(n))
...
export async function generateTargetRealisasiExcel(
    ...
    if (tabel === 'matrix') {
        ...
        const headers = ['No', 'Nama Retribusi', 'Target', ...BULAN_NAMES_SHORT, 'Total Penerimaan', '% Realisasi']
        ...
        // === DATA ROWS ===
        let totalTarget = 0
        let totalBulanan: number[] = Array(12).fill(0)
        let grandTotal = 0
        ...
        matrixData.forEach((row, idx) => {
            ...
            values.forEach((v, ci) => {
                ...
                if (ci === values.length - 1) {
                    ...
                    cell.font = { color: { argb: pct >= 100 ? 'FF1f7a1f' : pct >= 70 ? 'FF854d0e' : 'FFdc2626' }, bold: true }
                }
            })
            dataRow.height = 20
        })
        ...
        const headers = [
            ...
            'Realisasi s/d Bln Ini', '% Pencapaian', 'Sisa Target',
        ]
        const headerRow = ws.getRow(2)
        headers.forEach((h, i) => {
            const cell = headerRow.getCell(i + 1)
        ...
        rekapData.forEach((row, idx) => {
            ...
            const dataRow = ws.getRow(idx + 3)
            const values = [
                idx + 1, row.namaRetribusi, row.target,
                row.realisasiBulanLalu, row.realisasiBulanIni, row.realisasiTotal,
                `${row.persentase.toFixed(2)}%`, row.sisaTarget,
            ]
            values.forEach((v, ci) => {
                const cell = dataRow.getCell(ci + 1)
                cell.value = v
        ...
        const totalValues = [
            '', 'TOTAL', totals.target,
            totals.bulanLalu, totals.bulanIni, totals.total,

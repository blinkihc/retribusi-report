...
const formatRpFull = (n: number): string => new Intl.NumberFormat('id-ID').format(Math.round(n))
...
export async function generateTargetRealisasiPdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (tabel === 'matrix') {
      ...
      let totalTarget = 0
      const totalBulanan = Array(12).fill(0)
      let grandTotal = 0
      ...
      matrixData.forEach((row, idx) => {
        ...
        values.forEach((v, ci) => {
          ...
          if (isPercent) {
            doc
              .fill(pct >= 100 ? '#1f7a1f' : pct >= 70 ? '#854d0e' : '#dc2626')
              .font('Helvetica-Bold')
          }
      ...
      const totalValues = [
        '',
        'TOTAL',
        formatRpFull(totals.target),
        formatRpFull(totals.bulanLalu),
        formatRpFull(totals.bulanIni),
        formatRpFull(totals.total),

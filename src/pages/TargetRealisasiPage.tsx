...
import { type TargetRealisasiMatrix, type TargetRealisasiRekap } from '@/lib/db/schema'
...
function BadgePct({ pct }: { pct: number }) {
    return (
        <span className={`inline-block px-2 py-0.5 rounded border text-sm font-semibold ${pctBadge(pct)}`}>
            {pct.toFixed(2)}%
        </span>
    )
}
...
function TabelMatrix({ data, tahun, onExportExcel, onExportPdf, loading }: MatrixTableProps) {
    ...
    if (loading) return <div className="flex items-center justify-center py-16"><Loader2 className="h-8 w-8 animate-spin text-slate-400" /></div>
                ...
                <table className="min-w-full text-sm">
                    <thead>
                        <tr className="bg-blue-700 text-white">
                            ...
                            {BULAN.map((b) => (
                                <th key={b} className="text-right px-2 py-3 font-semibold min-w-[85px] whitespace-nowrap border-r border-blue-600">
                                    {b}
                                </th>
                            ))}
                            <th className="text-right px-3 py-3 font-semibold min-w-[110px] whitespace-nowrap border-r border-blue-600">
                        ...
                        {data.map((row, idx) => (
                            ...
                            >
                                ...
                                {Array.from({ length: 12 }, (_, bi) => {
                                    ...
                                    return (
                                        <td key={bi} className="text-right px-2 py-2.5 border-r border-slate-100 whitespace-nowrap">
                                            {val > 0 ? (
                    ...
                    <tfoot>
                        <tr className="bg-blue-50 border-t-2 border-black font-bold">
                            ...
                            {Array.from({ length: 12 }, (_, bi) => (
                                <td key={bi} className="text-right px-2 py-3 text-slate-900 border-r border-slate-200 whitespace-nowrap">
                                    {totals.bulanan[bi + 1] > 0 ? formatNum(totals.bulanan[bi + 1]) : <span className="text-slate-300">-</span>}
                                </td>
                            ))}
                            <td className="text-right px-3 py-3 text-slate-900 border-r border-slate-200 whitespace-nowrap">
...
function TabelRekap({ data, tahun, namaBulan, onExportExcel, onExportPdf, loading }: RekapTableProps) {
    const totals = useMemo(() => ({
                ...
                <table className="min-w-full text-sm">
                        ...
                        {data.map((row, idx) => (
                            ...
                            >
                                ...
                                <td className="text-right px-3 py-2.5 text-slate-600 border-r border-slate-100 whitespace-nowrap">
                                    {row.realisasiBulanLalu > 0 ? formatRp(row.realisasiBulanLalu) : <span className="text-slate-300">-</span>}
                                </td>
                                <td className="text-right px-3 py-2.5 text-slate-700 border-r border-slate-100 whitespace-nowrap">
                                    {row.realisasiBulanIni > 0 ? formatRp(row.realisasiBulanIni) : <span className="text-slate-300">-</span>}
                                </td>
                                <td className="text-right px-3 py-2.5 font-semibold text-slate-800 border-r border-slate-100 whitespace-nowrap">
                                    {row.realisasiTotal > 0 ? formatRp(row.realisasiTotal) : <span className="text-slate-400">Rp0</span>}
                                </td>
                                <td className="text-center px-3 py-2.5 border-r border-slate-100 whitespace-nowrap">
                                    <BadgePct pct={row.persentase} />
                                </td>
                                <td className={`text-right px-3 py-2.5 font-semibold whitespace-nowrap ${row.sisaTarget < 0 ? 'text-green-700' : 'text-red-600'}`}>
                                    {row.sisaTarget < 0 ? `+${formatRp(Math.abs(row.sisaTarget))}` : formatRp(row.sisaTarget)}
                    ...
                    <tfoot>
                        <tr className="bg-slate-100 border-t-2 border-black font-bold">
                            ...
                            <td className="text-right px-3 py-3 text-slate-900 border-r border-slate-200 whitespace-nowrap">{formatRp(totals.target)}</td>
                            <td className="text-right px-3 py-3 text-slate-900 border-r border-slate-200 whitespace-nowrap">{formatRp(totals.bulanLalu)}</td>
                            <td className="text-right px-3 py-3 text-slate-900 border-r border-slate-200 whitespace-nowrap">{formatRp(totals.bulanIni)}</td>
                            <td className="text-right px-3 py-3 text-slate-900 border-r border-slate-200 whitespace-nowrap">{formatRp(totals.total)}</td>
                            <td className="text-center px-3 py-3 border-r border-slate-200 whitespace-nowrap">
                                <BadgePct pct={totalPct} />
                            </td>
                            <td className={`text-right px-3 py-3 whitespace-nowrap ${totals.sisa < 0 ? 'text-green-700' : 'text-red-700'}`}>
                                {totals.sisa < 0 ? `+${formatRp(Math.abs(totals.sisa))} (lebih)` : formatRp(totals.sisa)}
                            </td>
                        </tr>
                    </tfoot>
                </table>
...
function InputTargetPanel({ tahun, existingTargets, onSaved }: InputTargetPanelProps) {
    ...
    return (
        <div className="bg-white border-2 border-black rounded-xl p-5 shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
            ...
            <div className="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                        Cari retribusi → isi nominal → klik Tambah. Klik Simpan Semua untuk menyimpan ke database.
                    </p>
                </div>
                <button
                    onClick={() => saveBulk()}
            ...
            <div className="flex gap-2 mb-4 items-end">
                ...
                <div className="flex-1 relative">
                    ...
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            value={selectedOption ? `${selectedOption.kode} – ${selectedOption.nama}` : searchQuery}
                            onChange={(e) => {
                        ...
                        {(searchQuery || selectedOption) && (
                            <button
                                ...
                                onClick={() => { setSearchQuery(''); setSelectedOption(null); setDebouncedSearch(''); }}
                        ...
                        {showDropdown && !selectedOption && debouncedSearch.length >= 1 && (
                            <div className="absolute top-full left-0 right-0 z-50 mt-1 max-h-56 overflow-y-auto bg-white border border-slate-200 rounded-lg shadow-lg">
                                ...
                                {!searching && searchOptions.length === 0 && (
                                    <div className="px-3 py-3 text-slate-400 text-sm">Tidak ada hasil untuk "{debouncedSearch}"</div>
                                )}
                ...
                {/* Nominal input */}
                <div className="w-52">
                    <label className="block text-xs font-medium text-slate-600 mb-1">Target Nominal (Rp)</label>
                    <input
                        type="text"
                        inputMode="numeric"
                        value={nominalFocused ? (nominalVal === 0 ? '' : String(nominalVal)) : (nominalVal === 0 ? '' : formatAngka(nominalVal))}
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-slate-100 border-b border-slate-200">
                            ...
                            <th className="text-right px-4 py-2.5 font-semibold text-slate-700 w-52">Target Nominal (Rp)</th>
                        ...
                        {drafts.map((d, idx) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <td className="px-3 py-2.5 text-slate-500 text-xs font-mono">{d.kodeRetribusi}</td>
                                    <td className="px-4 py-2.5">
                                        {isEditing ? (
                                            <input
                                                ...
                                                type="text"
                                                inputMode="numeric"
                                                value={editFocused ? (parseAngka(editNominal) === 0 ? '' : String(parseAngka(editNominal))) : (parseAngka(editNominal) === 0 ? '' : formatAngka(parseAngka(editNominal)))}
                                            ...
                                            >
                                                {d.targetNominal > 0 ? formatAngka(d.targetNominal) : <span className="text-slate-300">0 (klik edit)</span>}
                                            </button>
                                        )}
                                    </td>
            ...
            {drafts.length > 0 && (
                <p className="text-xs text-slate-400 mt-2">{drafts.length} retribusi dalam daftar · Klik nominal untuk edit</p>
            )}
        </div>
    )
}
        ...
        <div className="space-y-6 pb-8">
            ...
            {rekapData.length > 0 && (
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        { label: 'Total Target', value: formatNum(totalTarget), color: 'border-blue-400 bg-blue-50' },
                        { label: 'Total Realisasi', value: formatNum(totalRealisasi), color: 'border-green-400 bg-green-50' },
                        {
                            label: '% Pencapaian',
                            value: `${totalPct.toFixed(2)}%`,
                            color: totalPct >= 100 ? 'border-green-400 bg-green-50' : totalPct >= 70 ? 'border-yellow-400 bg-yellow-50' : 'border-red-400 bg-red-50',
                        },
                        {
                            label: 'Sisa Target',
